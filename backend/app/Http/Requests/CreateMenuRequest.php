 ...
 * メニュー作成リクエスト
 * 
 * メニューとオプションの作成時バリデーション
 * 業種別制約、価格・時間の妥当性チェックを含む
 */
class CreateMenuRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // 価格・時間
            'base_price' => 'required|integer|min:0|max:999999',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            // 総時間の妥当性チェック
            $baseDuration = $this->base_duration ?? 0;
            $prepDuration = $this->prep_duration ?? 0;
            $cleanupDuration = $this->cleanup_duration ?? 0;
