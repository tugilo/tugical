...
- **確認結果（2026-07-07 19:50:00 — ローカル確認）**:
  - **端末で完走**: **No（ブロック中）** — store `line_liff_id` 未設定、`http://localhost/liff/1` は空白画面
  - **管理画面に反映**: 未実施（予約未作成のため）
  - **次のアクション**: Settings で LIFF ID 設定 → LINE 内ブラウザまたは LIFF URL で再試行
...
- **確認結果（2026-07-07 19:49:38 — 再チェック）**:
  - **結合テスト**: Yes — `LineBookingNotificationTest` 3 passed
  - **事前チェック**: `tugical:verify-line-e2e --store=1` → ✗ line_channel / ✗ access_token / ✗ line_customer（**実機 E2E は LINE Developers 設定待ち**）
  - **管理画面からの設定**: Settings ウィザード +「認証情報を確認」実装済（`644c6b1`）。チャネル情報入力後に再実行可能
