...
const MenuEditModal: React.FC<MenuEditModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.base_duration !== undefined) {
      const baseDuration =
        typeof formData.base_duration === 'string'
          ? parseFloat(formData.base_duration)
          : formData.base_duration;
    ...
    if (
      formData.cleanup_duration !== undefined &&
      formData.cleanup_duration < 0
    ) {
    ...
    if (formData.base_duration !== undefined) {
      const baseDuration =
        typeof formData.base_duration === 'string'
          ...
          : formData.base_duration;
      const prepDuration =
        formData.prep_duration !== undefined
          ? typeof formData.prep_duration === 'string'
            ? parseFloat(formData.prep_duration)
            : formData.prep_duration
          : originalMenu?.prep_duration || 0;
      const cleanupDuration =
        formData.cleanup_duration !== undefined
          ? typeof formData.cleanup_duration === 'string'
            ? parseFloat(formData.cleanup_duration)
  ...
  return (
    <Modal
      ...
      title={
        originalMenu
          ? `メニュー編集: ${originalMenu.display_name}`
          : 'メニュー編集'
      }
      size='lg'
          ...
          <div className='space-y-4'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              ...
              <FormField
                ...
                type='number'
                value={
                  formData.base_duration !== undefined
                    ? formData.base_duration
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              ...
              <div className='flex items-center space-x-3'>
                <input
                  ...
                  checked={
                    formData.requires_approval !== undefined
                      ? formData.requires_approval
                      : originalMenu.requires_approval
                  }
                  onChange={e =>
